...
public class ItemRunicArmorPrimal extends ItemRunicArmor
    implements IRevealer, IGoggles, IWarpingGear, IVisDiscountGear {
    ...
    public int getRunicCharge(ItemStack itemstack) {
        // return (this.armorType == 0) ? itemstack.getMaxDamage() : ((this.armorType == 1) ? itemstack.getMaxDamage() :
        // ((this.armorType == 2) ? itemstack.getMaxDamage() : itemstack.getMaxDamage()));
        // return 0;
        ...
        if ((getUpgrade(itemstack) == 2) || (getUpgrade2(itemstack) == 2) || (getUpgrade3(itemstack) == 2)) {
            if ((getUpgrade(itemstack) == 2) && (getUpgrade2(itemstack) == 2)
                ...
                if ((getUpgrade(itemstack) == 2) && (getUpgrade2(itemstack) == 2) && (getUpgrade3(itemstack) == 2)) {
                    return (this.armorType == 0) ? 20
                        : ((this.armorType == 1) ? 60 : ((this.armorType == 2) ? 40 : 20)); // 140 total. what madman
                                                                                            // would actually do this?
                }

                else {
                    return (this.armorType == 0) ? 16
                        : ((this.armorType == 1) ? 48 : ((this.armorType == 2) ? 32 : 16)); // 112 total
                }
            } else {
                return (this.armorType == 0) ? 12 : ((this.armorType == 1) ? 36 : ((this.armorType == 2) ? 24 : 12)); // 84
                                                                                                                      // total
            }
        }
    ...
    public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage,
         ...
         * //Primal's higher base armour (if I keep it, keep changing my mind) should discourage people from stacking
         * this one anyway
         * }
         * else {
         * priority = 1;
        ...
        if (event.entity instanceof EntityPlayer) {
            ...
            for (int a = 0; a < 4; a++) {
                if (player.inventory.armorItemInSlot(a) != null && (player.inventory.armorItemInSlot(a)
                    .getItem() instanceof ItemRunicArmorPrimal)) {
                    set++;
                }
            }
    ...
    public Multimap getAttributeModifiers(ItemStack armor) {
        ...
        // map.put(SharedMonsterAttributes.knockbackResistance.getAttributeUnlocalizedName(),new AttributeModifier(uuid,
        // "Runic knockback", this.getArmorDisplay(null, armor, aType) / 20.0, 0));
    ...
    public String getArmorTexture(ItemStack stack, Entity entity, int slot, String type) {
        if (entity instanceof EntityPlayer) {
            ...
            if (player.getUniqueID()
                .toString() == "8c3dbe36-8161-4207-a2d6-e303dfe260ba") { // because dev perks. purely asstheticc
