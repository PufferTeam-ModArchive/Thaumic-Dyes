...
public class ItemRunicArmor extends ItemArmor implements ISpecialArmor, IRunicArmor {
    ...
    public IIcon getIconFromDamage(final int par1) {
        return (this.armorType == 0) ? this.iconHelm
            : ((this.armorType == 1) ? this.iconChest : ((this.armorType == 2) ? this.iconLegs : this.iconBoots));
    }
    ...
    public int getRunicCharge(ItemStack itemstack) {
        ...
        if ((itemstack.hasTagCompound()) && (itemstack.stackTagCompound.hasKey("upgrade"))
            && (itemstack.stackTagCompound.getInteger("upgrade") == 2)) {
            return (this.armorType == 0) ? 12 : ((this.armorType == 1) ? 36 : ((this.armorType == 2) ? 24 : 12)); // TODO
                                                                                                                  // 84
                                                                                                                  // total,
                                                                                                                  // TBA
        }
    ...
    /*
     * public ArmorProperties getProperties(final EntityLivingBase player, final ItemStack armor, final DamageSource
     * source, final double damage, final int slot) {
    ...
    public Multimap getAttributeModifiers(ItemStack armor) {
        ...
        // "Abyssal modifier " + aType, this.getArmorDisplay(null, armor, aType) / 20.0, 1));
            ...
            if (armor.getItemDamage() > 0) {
                ...
                for (int a = 0; a < 4; ++a) {
                    if (player.inventory.armorItemInSlot(a) != null && player.inventory.armorItemInSlot(a)
                        .getItem() instanceof ItemRunicArmor && getUpgrade(player.inventory.armorItemInSlot(a)) == 1) {
                        rate -= 150;
                    }
                }
                if (rate < 200) {
