...
public class IronFortressArmor extends ItemArmor
    implements IRepairable, IRunicArmor, ISpecialArmor, IGoggles, IRevealer, IWarpingGear {
        ...
        if ((stack.hasTagCompound()) && (stack.stackTagCompound.hasKey("mask"))) {
            list.add(
                EnumChatFormatting.GOLD + StatCollector.translateToLocal(
                    new StringBuilder().append("item.HelmetIronFortress.mask.")
    ...
    public int getWarp(ItemStack stack, EntityPlayer player) {
        if ((stack.hasTagCompound()) && (stack.stackTagCompound.hasKey("mask"))
            && (stack.stackTagCompound.getInteger("mask") == 2)) {
            return 1;
        }
        return 0;
    }
