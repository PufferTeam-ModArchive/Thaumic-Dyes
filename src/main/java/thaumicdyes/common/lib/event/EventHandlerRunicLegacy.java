    ...
    public void entityHurt(final LivingHurtEvent event) {
        if (event.entity != null) {
            if (event.entity instanceof EntityPlayerMP && (!(event.entity instanceof FakePlayer))) {
                ...
                // int runicTotal =
                // (Thaumcraft.instance.runicEventHandler.runicInfo.get(Integer.valueOf(player.getEntityId())))[0].intValue();
                int runicCharge = (Thaumcraft.instance.runicEventHandler.runicCharge
                    .get(Integer.valueOf(player.getEntityId()))).intValue();

                for (int a = 0; a < 4; a++) {
                    if (player.inventory.armorItemInSlot(a) != null && (player.inventory.armorItemInSlot(a)
                        .getItem() instanceof ItemRunicArmor)) { // this should cover the enhanced one too
                    ...
                    else if ((player.inventory.armorItemInSlot(a) != null) && ((player.inventory.armorItemInSlot(a)
                        .getItem() instanceof ItemRunicArmor))) {
                ...
                if (runic.size() > 0) {
                    ...
                    if (runicCharge <= 0 && berserker > 0 && !this.upgradeCooldown.containsKey(key)) {
                        ...
                        if (berserker >= 4) {
                            player.addPotionEffect(
                                new PotionEffect(Potion.resistance.id, 200 + (100 * (berserker - 4)), berserker - 4));
                        }
                    ...
                    if (runicCharge <= 0 && kinetic > 0 && !this.upgradeCooldown.containsKey(key)) {
                        ...
                        player.worldObj.newExplosion(
                            ...
                            false,
                            false);
                        // player.worldObj.newExplosion((Entity)player, player.posX, player.posY + player.height / 2.0f,
                        // player.posZ, 1.5f + this.runicInfo.get(player.getEntityId())[2] * 0.5f, false, false);
                    ...
                    if (runicCharge <= 0 && emergency > 0 && !this.upgradeCooldown.containsKey(key)) {
                        ...
                        runicCharge = Math
                            .min(Thaumcraft.instance.runicEventHandler.runicInfo.get(player.getEntityId())[0], t);
                    ...
                    PacketHandler.INSTANCE.sendTo(
                        (IMessage) new PacketRunicCharge(
                            player,
                            (short) runicCharge,
                            Thaumcraft.instance.runicEventHandler.runicInfo.get(player.getEntityId())[0]),
